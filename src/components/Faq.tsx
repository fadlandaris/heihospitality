...
export default function Faq() {
  ...
  return (
    <section className='py-24 border-b border-border border-dashed'>
      ...
      {/* <div className='  pb-24 border-b border-border border-dashed'> */}
        <div className='max-w-3xl mt-16 mx-auto bg-accent/10 rounded-lg p-3 grid grid-cols-1 gap-y-1'>
          {faqData.map((item) => {
              ...
              <div key={item.id} className='bg-white rounded-lg'>
                ...
                >
                  <div className='flex items-center justify-between'>
                    ...
                    <span className={`transition-transform duration-300 ${open ? 'rotate-45 text-foreground' : 'text-accent'}`}>
                      <PlusIcon size={16} />
                    </span>
                  </div>
                </div>
                <div className={`grid transition-all duration-300 ease-out ${open ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
                  <div className='overflow-hidden'>
                    <p className='px-3 pb-4 text-accent'>
                      {item.desc}
                    </p>
                  </div>
                </div>
